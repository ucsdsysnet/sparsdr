...
///
/// This implementation does not include gaps between samples, but it does pass through timeouts
/// to allow flushing samples.
pub struct OverlapFlush<I> {
    /// Inner iterator
    inner: Fuse<I>,
...
impl<I> Iterator for OverlapFlush<I>
where
    I: Iterator<Item = Status<TimeWindow>>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            match self.inner.next() {
                ...
                Some(Status::FirstWindowTime(_)) => {
                    // Ignore (this is used only with the overlap-gasp version)
                }
...
mod test {
    ...
    {
        let overlap = OverlapFlush::new(windows.into_iter(), window_size);
        let result = overlap
            .map(|ow| ow.window)
            .flatten()
